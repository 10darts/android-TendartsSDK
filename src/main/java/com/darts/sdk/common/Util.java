...
public class Util
{
	...
	public static  void setBadgeCount( int count, Context context)
	{
		try
		{
			...
			intent.putExtra("badge_count_class_name", DartsClient.instance(context).mainActivityClassName());
		...
		try
		{
			...
			intent.putExtra("com.sonyericsson.home.intent.extra.badge.ACTIVITY_NAME", DartsClient.instance(context).mainActivityClassName());
		...
		if( _provider == null)
		{
			...
			new Communications.ICommunicationsConfigProvider()
			{
				...
				public void onGeostatSent(boolean success, String info)
				{
					if (success)
					{
						DartsClient.instance(context).logEvent("GEO", "Succesfully  sent geoStats", "");
					} else
					{
						DartsClient.instance(context).logEvent("GEO", "Failed to send geoStats", "" + info);
					}
				}
			};
		}
	...
	public static void checkUnauthorized(String reason, Context context)
	{
		...
		{
			DartsClient.instance(context).remoteLogException(new Exception(reason));
		}
	}
}
